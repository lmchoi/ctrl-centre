...
function parseTaskLine(line) {
  ...
  return {
    ...
    text: rest.trim(),
    priority: priorityMatch ? priorityMatch[1].toLowerCase() : DEFAULT_PRIORITY,
...
function resolveTask(doc, ordinal, expectedText) {
  ...
  if (!task) {
    const err = new Error('That task no longer exists — the file changed. Reloading.');
    err.status = 409;
    throw err;
  }
  if (typeof expectedText === 'string' && task.text !== expectedText) {
    const err = new Error('That task changed on disk — reloading so you do not edit the wrong one.');
    err.status = 409;
    throw err;
  }
  return task;
...
export function createTodoStore(filePath) {
  ...
  return {
    ...
    add({ text, priority, due }) {
      const trimmed = String(text ?? '').trim();
      if (!trimmed) {
        const err = new Error('Task text is required.');
        err.status = 400;
        throw err;
      }
      const normalizedPriority = PRIORITIES.includes(priority) ? priority : DEFAULT_PRIORITY;
      const normalizedDue = DUE_RE.test(` @${due}`) ? due : '';
      ...
      return mutate((doc) => {
        ...
        while (doc.lines.length && doc.lines[doc.lines.length - 1].trim() === '') doc.lines.pop();
